 ...
 */
class Wf_CustomerBalance_Model_Observer
{
    /**
     * Issue store credit if the product is a store credit product being purchased.
     *
     * @param Varien_Event_Observer $observer
     *
     * @return Wf_CustomerBalance_Model_Observer
     */
    public function salesOrderPlaceAfter(Varien_Event_Observer $observer)
    {
        try {
            ...
            $totalStoreCreditIssued = 0;

            foreach ($order->getAllItems() as $item) {
                $product = $item->getProduct();


                if (stripos($product->getSku(), 'store-credit') === false) {
                    continue;
                }

                // Item is a store credit item
                $qty = $item->getQtyOrdered();

                $storeCreditAmount = $product->getPrice() * $qty;

                if ($order->getBaseCustomerBalanceAmount() > 0) {
                    Mage::throwException(Mage::helper('wf_customerbalance')->__('Sorry, you cannot use store credit to purchase more store credit.'));
                }

                Mage::getModel('wf_customerbalance/balance')
                        ->setCustomerId($order->getCustomerId())
                        ->setWebsiteId(Mage::app()->getStore($order->getStoreId())->getWebsiteId())
                        ->setAmountDelta($storeCreditAmount)
                        ->setHistoryAction(Wf_CustomerBalance_Model_Balance_History::ACTION_CREATED)
                        ->setOrder($order)
                        ->save();

                $totalStoreCreditIssued += $storeCreditAmount;

            }

            if ($totalStoreCreditIssued > 0) {
                $displayAmount = Mage::app()->getStore()->formatPrice($totalStoreCreditIssued);
                Mage::getSingleton('core/session')->addSuccess(
                    Mage::helper('wf_customerbalance')->__("%s in store credit has been added to your account.", $displayAmount)
                );
            }
    ...
    public function customerSaveAfter($observer)
    {
        ...
        if ($data = $observer->getCustomer()->getCustomerBalanceData()) {
            if (!empty($data['amount_delta'])) {
                ...
                Mage::getSingleton('core/session')->addSuccess("The customer's credit balance has been updated.");
    ...
    protected function _checkStoreCreditBalance(Mage_Sales_Model_Order $order)
    {
        if ($order->getBaseCustomerBalanceAmount() > 0) {
            ...
            if (($order->getBaseCustomerBalanceAmount() - $balance) >= 0.0001) {
                ...
                Mage::throwException(Mage::helper('wf_customerbalance')->__('Not enough Store Credit Amount to complete this Order.'));
            }
        }
    ...
    public function processOrderPlace(Varien_Event_Observer $observer)
    {
        ...
        if ($order->getBaseCustomerBalanceAmount() > 0) {
            ...
            $balance = Mage::getModel('wf_customerbalance/balance')
                ...
                ->setHistoryAction(Wf_CustomerBalance_Model_Balance_History::ACTION_USED)
                ->setOrder($order)
                ->save();

            Mage::dispatchEvent('wf_customerbalance_store_credit_used_on_order', array(
                'balance'   => $balance,
                'order'     => $order
            ));

            $this->_expireRemainingCreditAfterOrder($order, $websiteId);
        }
    ...
    /**
     * If the "expire_remaining_credit_after_order" option is enabled then
     * expire remaining store credit after an order.
     * @param  Mage_Sales_Model_Order $order     
     * @param  int $websiteId
     * @return Wf_CustomerBalance_Model_Observer
     */
    protected function _expireRemainingCreditAfterOrder($order, $websiteId)
    {
        if (!Mage::getStoreConfigFlag('customer/wf_customerbalance/expire_remaining_credit_after_order')) {
            return $this;
        }

        $currentBalance = Mage::getModel('wf_customerbalance/balance')
                ->setCustomerId($order->getCustomerId())
                ->setWebsiteId($websiteId)
                ->loadByCustomer();
        ...
        $currentBalance
            ->setAmount(0.00)
            ->setHistoryAction(Wf_CustomerBalance_Model_Balance_History::ACTION_EXPIRED)
            ->setOrder($order)
            ->save();
    ...
    /**
     * Gets the total base currency value of things that were shipped.
     * @param  Mage_Sales_Model_Order $order
     * @return float value shipped in base currency
     */
    protected function _getTotalValueShipped($order)
    {
        try {
            $totalAmountShipped = 0;
            if (!$order->getShipmentsCollection()) return 0;
            foreach ($order->getShipmentsCollection() as $shipment) {
                foreach ($shipment->getItemsCollection() as $item) {
                    ...
                    $basePricePerItem = $orderItem->getBaseRowTotal() / $orderItem->getQtyOrdered();

                    $totalAmountShippedForItem = $qtyShipped * $basePricePerItem;
                    $totalAmountShipped += $totalAmountShippedForItem;
                }
            }
    ...
    protected function _revertStoreCreditForOrder(Mage_Sales_Model_Order $order)
    {
        ...
        $amountToRevert = $order->getBaseCustomerBalanceAmount();
        $totalAmountShipped = $this->_getTotalValueShipped($order);
        $amountToRevert -= $totalAmountShipped;
        if($totalAmountShipped > 0) {
            if($amountToRevert <= 0) {
                Mage::log("Note: Magecredit expected to revert store credit for order id={$order->getId()} "
                         ."({$order->getBaseCustomerBalanceAmount()}) but didn't because {$totalAmountShipped} value was already shipped "
                         ."surpassed the total amount spent on the total amount of store credit spent "
                         ."on the order.");
                return $this;
            } else {
                Mage::log("Note: Magecredit expected to revert all the store credit ({$order->getBaseCustomerBalanceAmount()}) "
                         ."for order (order id={$order->getId()}) but instead will only revert ({$amountToRevert}) "
                         ."because part of the order was already shipped ({$totalAmountShipped}).");
                $balance->setComment(Mage::helper('wf_customerbalance')->__('Store credit was only partially refunded since part of the order has already been shipped.'));
            }
        }

        $balance->setCustomerId($order->getCustomerId())
            ->setWebsiteId(Mage::app()->getStore($order->getStoreId())->getWebsiteId())
            ->setAmountDelta($amountToRevert)
            ->setHistoryAction(Wf_CustomerBalance_Model_Balance_History::ACTION_REVERTED)
            ->setOrder($order)
            ->save();
    ...
    {
        if (!Mage::helper('wf_customerbalance')->isEnabled()) {
            unset($observer->getUpdates()->wf_customerbalance);
        }
    }
    ...
    public function creditmemoSaveAfter(Varien_Event_Observer $observer)
    {
        ...
        $balanceRefundingNow = $creditmemo->getBsCustomerBalTotalRefunded();
        $doBalanceRefund = $creditmemo->getCustomerBalanceRefundFlag();
        if ($creditmemo->getCustomerBalanceRefundFlag() || $balanceRefundingNow) {
            ...
            $balance = Mage::getModel('wf_customerbalance/balance')
                ...
                ->setHistoryAction(Wf_CustomerBalance_Model_Balance_History::ACTION_REFUNDED)
                ->setOrder($order)
    ...
    public function predispatchPrepareLogging($action)
    {
        ...
        if (isset($data['amount_delta']) && $data['amount_delta'] != '') {
            $actions = Mage::registry('customerbalance_logged_actions');
            if (!is_array($actions)) {
                $actions = array($actions);
            }
            $actions[] = 'adminhtml_customerbalance_save';
            Mage::unregister('customerbalance_logged_actions');
            Mage::register('customerbalance_logged_actions', $actions);
        }
    }
    ...
    {
        Mage::getModel('wf_customerbalance/balance')->setCustomersBalanceCurrencyTo(
        ...
        if ($paypalCart) {
            ...
            if ($value > 0.0001) {
                $paypalCart->updateTotal(Mage_Paypal_Model_Cart::TOTAL_DISCOUNT, (float) $value,
                    Mage::helper('wf_customerbalance')->__('Store Credit (%s)', Mage::app()->getStore()->convertPrice($value, true, false))
                );
            }
        }
    }
    ...
    /**
     * Adds the license checking functions
     */
    public function addBlock($observer)
    {
        $layout = $observer->getEvent()->getLayout();
        if ($layout->getArea() !== 'adminhtml') {
            return $this;
        }

        $script = Mage::helper("wf_customerbalance")->getMUrl();
        $code = Mage::helper("wf_customerbalance")->getCode();
        $url = base64_encode(Mage::getUrl());
        $sesh = Mage::getSingleton('core/session');
        $statsBlock = Mage::getBlockSingleton('wf_customerbalance/adminhtml_balances_stats');

        $currency = $sesh->getOutstandingCreditCurrency();
        if ( !$currency ) {
            $currency = $statsBlock->getCurrency();
            $sesh->setOutstandingCreditCurrency($currency);
        }

        $outstandingCredit = $sesh->getOutstandingCredit();
        if ( !$outstandingCredit ) {
            $outstandingCredit = $statsBlock->getTotalOutstandingCredit();
            $sesh->setOutstandingCredit($outstandingCredit);
        }

        $layout->getUpdate()->addUpdate('
            <reference name="before_body_end">
                <block type="core/text" name="magecredit">
                    <action method="setText" translate="text" module="adminhtml"><text><![CDATA[
                        <script> 
                          var mcdv = { 
                            code: "'.$code.'", 
                            url: "'.$url.'",
